...
▢ ${msg.gp} 1
https://chat.whatsapp.com/L8KjsGZDmcTFhzR6vladV3
...
case 'creador': 
case 'developer':
number = '51972953400@s.whatsapp.net'
    capt = `▢ ${msg.num} : @${number.split('@')[0]}\n\n`
    capt += '▢ Tik Tok : https://vm.tiktok.com/ZM87sEVdx/'
    await Fg.fakeLink(from, capt, thumbfg, `${msg.click}`, 'https://vm.tiktok.com/ZM87sEVdx/', mek)
...
*≡ OWNER*
▢ Tik Tok : https://vm.tiktok.com/ZM87sEVdx/
▢ WhatsApp : wa.me/51972953400

*≡ CREADOR*
▢ By : ${package.homepage} 
` 
Fg.send2Button(from, teks, '*_© EFRAIN_*', `ꨄ︎ Apoyar`, `${prefix}donate`, `⌬ ${msg.gp}s`, `${prefix}grupos`)
   break
...
▢ ${msg.tamaño} : ${resv.size}`
 ...
 //   if(!isOwner && !isBot) return
   if(!isPremium && !isOwner && !isBot) return m.reply(msg.premium)
    ...
    if(!isGroup) return m.reply(msg.group)
    if(
      !isBotAdmins) return m.reply(msg.botadmin)
...
Fg.send2ButtonLoc(from, welm, capt, 'Sígueme en Tik Tok\nhttps://vm.tiktok.com/ZM87sEVdx/', '⦙☰ MENU', '/menu', '⏍ INFO GP', '/infogp', false, {
	      contextInfo: { 
            mentionedJid: Fg.parseMention(capt)
	      } 
	    }); //--
  ...
  //Fg.sendMessage(from, welm, image, {contextInfo: {  mentionedJid: [sender]}, thumbnail: fakethumb, quoted: mek, caption: capt})
   Fg.sendButtonLoc(from, welm, capt, 'Sígueme en Tik Tok\nhttps://vm.tiktok.com/ZM87sEVdx/', '👋🏻', 'unde', false, {
	      contextInfo: { 
            mentionedJid: Fg.parseMention(capt)
	      } 
	    });//---
